            ...
            <div class="user-results-stack">
                <?php foreach ($evaluationResults as $evaluationResult): ?>
                    ...
                    <article class="user-result-card">
                        ...
                        <?php if ($media || $screenCaptures): ?><template id="<?= e($videoDrawerId) ?>"><?php foreach ($media as $mediaSegment): ?><?php $mediaStatus = (string) ($mediaSegment['status'] ?? ''); ?><div class="border rounded p-3 mt-3"><h4 class="h6 fw-bold mb-2">Grabación <?= (int) ($mediaSegment['segment_number'] ?? 1) ?></h4><?php if (in_array($mediaStatus, ['saved', 'partial'], true)): ?><video class="w-100 rounded border" controls preload="metadata" src="<?= e(route_url('evaluation-surveys.attempt.media.evidence', $attemptId) . '?evidence_id=' . (int) $mediaSegment['id']) ?>"></video><p class="text-muted small mt-2 mb-0">Duración: <?= (int) ($mediaSegment['duration_seconds'] ?? 0) ?> segundos.</p><?php if ($mediaStatus === 'partial'): ?><div class="alert alert-warning mt-2 mb-0">Evidencia parcial.</div><?php endif; ?><?php else: ?><div class="alert alert-warning mb-2"><strong>Evidencia pendiente de procesamiento.</strong><br><span class="small"><?php if (!empty($mediaSegment['processing_job_status'])): ?>Job automático: <?= e((string) $mediaSegment['processing_job_status']) ?><?php else: ?>No existe un job automático asociado a esta evidencia.<?php endif; ?><?php if (!empty($mediaSegment['processing_job_error'])): ?><br><?= e((string) $mediaSegment['processing_job_error']) ?><?php endif; ?></span></div><form method="post" action="<?= e(route_url('evaluation-surveys.attempt.media.process', $attemptId)) ?>" class="d-inline"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="evidence_id" value="<?= (int) $mediaSegment['id'] ?>"><input type="hidden" name="return_url" value="<?= e((string) ($_SERVER['REQUEST_URI'] ?? '')) ?>"><button class="btn btn-sm btn-primary" type="submit"><i class="bi bi-arrow-repeat me-1"></i>Procesar evidencia ahora</button></form><?php endif; ?><?php $segmentCaptures = array_values(array_filter($screenCaptures, static fn(array $capture): bool => (int) ($capture['evidence_id'] ?? 0) === (int) ($mediaSegment['id'] ?? 0))); ?><?php if ($segmentCaptures): ?><h5 class="h6 fw-bold mt-3">Capturas de pantalla (<?= count($segmentCaptures) ?>)</h5><div class="row g-2 capture-gallery"><?php foreach ($segmentCaptures as $capture): $captureUrl = route_url('evaluation-surveys.attempt.media.screenshot-file', $attemptId) . '?capture_id=' . (int) $capture['id']; ?><div class="col-6"><a href="<?= e($captureUrl) ?>" data-screen-capture-view><img class="img-fluid rounded border" loading="lazy" src="<?= e($captureUrl) ?>" alt="Captura <?= (int) $capture['capture_number'] ?>"></a></div><?php endforeach; ?></div><?php endif; ?></div><?php endforeach; ?><div class="import-drawer-actions"><button class="btn btn-outline-secondary" type="button" data-app-drawer-close>Cerrar</button></div></template><?php endif; ?>
                    </article>
                    </details>
                <?php endforeach; ?>
            </div>
