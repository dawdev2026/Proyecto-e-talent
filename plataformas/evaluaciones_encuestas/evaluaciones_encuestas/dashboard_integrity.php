...
$reportDomain = (string) ($reportDomain ?? 'both');
$filterQuery = http_build_query(array_filter(['domain' => $reportDomain, 'date_from' => $dateFrom ?? '', 'date_to' => $dateTo ?? '', 'process_id' => $selectedProcessIds, 'form_id' => $selectedFormIds, 'test_id' => $selectedTestIds]));
...
<section class="page-header">
    <div><p class="dashboard-kicker mb-1">Reportes</p><h1 class="fw-bold mb-1"><?= $reportDomain === 'tests' ? 'Reporte Incidencia Test Psicolaborales' : 'Reporte Incidencia Evaluaciones' ?></h1><p class="text-muted mb-0">Señales registradas durante <?= $reportDomain === 'tests' ? 'tests psicolaborales' : 'evaluaciones' ?> asociadas a procesos. Requieren revisión autorizada y no constituyen por sí solas una conclusión de fraude.</p></div>
    <div class="d-flex flex-wrap gap-2"><a class="btn btn-outline-secondary" href="<?= e(route_url($reportDomain === 'tests' ? 'client-admin.test-progress' : 'evaluation-surveys.dashboard')) ?>"><i class="bi bi-arrow-left me-1"></i>Volver al dashboard</a></div>
</section>
<section class="card content-panel mb-4" aria-label="Filtros del reporte">
    <?= status_help_button('Estados e integridad del intento', "• Asignada: aún no se abre.\n• En curso: intento abierto; puede no tener respuestas.\n• Completada: enviado explícitamente.\n• Expirada: plazo vencido sin envío y no equivale a completada.\n• Cancelada: asignación retirada.\n• Las señales de integridad orientan una revisión autorizada; no son una conclusión automática.") ?>
    <form method="get" class="row g-2 align-items-end">
        <?php if ($reportDomain !== 'tests'): ?><div class="col-12 col-md-4"><label class="form-label" for="integrity-form-filter">Evaluación</label><select class="form-select js-integrity-select2" id="integrity-form-filter" name="form_id[]" multiple data-placeholder="Todas las evaluaciones"><?php foreach ((array) ($filterOptions['forms'] ?? []) as $optionId => $optionTitle): ?><option value="<?= (int) $optionId ?>" <?= in_array((int) $optionId, $selectedFormIds, true) ? 'selected' : '' ?>><?= e((string) $optionTitle) ?></option><?php endforeach; ?></select></div><?php endif; ?>
        <?php if ($reportDomain !== 'evaluations'): ?><div class="col-12 col-md-3"><label class="form-label" for="integrity-test-filter">Test psicolaboral</label><select class="form-select js-integrity-select2" id="integrity-test-filter" name="test_id[]" multiple data-placeholder="Todos los tests"><?php foreach ((array) ($filterOptions['tests'] ?? []) as $optionId => $optionTitle): ?><option value="<?= (int) $optionId ?>" <?= in_array((int) $optionId, $selectedTestIds, true) ? 'selected' : '' ?>><?= e((string) $optionTitle) ?></option><?php endforeach; ?></select></div><?php endif; ?>
        ...
        <div class="col-6 col-md-2"><label class="form-label" for="integrity-date-from">Desde</label><input class="form-control" type="date" id="integrity-date-from" name="date_from" value="<?= e((string) ($dateFrom ?? '')) ?>"></div>
        <div class="col-6 col-md-2"><label class="form-label" for="integrity-date-to">Hasta</label><input class="form-control" type="date" id="integrity-date-to" name="date_to" value="<?= e((string) ($dateTo ?? '')) ?>"></div>
        <div class="col-12 d-flex justify-content-end gap-2"><button class="btn btn-primary" type="submit">Filtrar</button><a class="btn btn-outline-secondary" href="<?= e(route_url($reportDomain === 'tests' ? 'client-admin.test-incidents' : 'evaluation-surveys.dashboard.integrity')) ?>" aria-label="Limpiar filtros">×</a></div>
    </form>
</section>
<section class="row g-3 mb-4" aria-label="Resumen ejecutivo de incidencias">
    <?php foreach ([['Asignaciones / sesiones','assigned_people','bi-people','primary'],['Personas distintas asignadas','distinct_assigned_people','bi-people-fill','info'],['Con intento / ejecución','attempted_people','bi-play-circle','info'],['Asignaciones / sesiones con incidencias','people_with_incidents','bi-flag','warning'],['Revisión','review_cases','bi-eye','warning'],['Alertas altas','high_alert_cases','bi-shield-exclamation','danger']] as [$label,$key,$icon,$color]): ?>
        <div class="col-12 col-sm-6 col-xl"><div class="card content-panel h-100"><div class="d-flex align-items-center gap-3"><span class="rounded-circle text-bg-<?= e($color) ?> p-2"><i class="bi <?= e($icon) ?>" aria-hidden="true"></i></span><div><span class="text-muted small d-block"><?= e($label) ?></span><strong class="fs-4"><?= $number($summary[$key] ?? 0) ?></strong></div></div></div></div>
    <?php endforeach; ?>
</section>
<section class="card content-panel mb-4">
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-3"><div><h2 class="h5 fw-bold mb-1">Resumen ejecutivo</h2><p class="text-muted mb-0">Análisis de <?= $reportDomain === 'tests' ? 'tests psicolaborales' : 'evaluaciones' ?> vinculadas a procesos de tu empresa, desde <?= e((string) ($dateFrom ?? '')) ?> hasta <?= e((string) ($dateTo ?? '')) ?>.</p></div><div class="text-end"><span class="text-muted small d-block">Eventos de actividad / audiovisuales</span><strong><?= $number($summary['activity_events'] ?? 0) ?> / <?= $number($summary['media_events'] ?? 0) ?></strong></div></div>
...
<section class="card content-panel">
    ...
    <div class="table-responsive"><table class="table table-hover align-middle app-table"><thead><tr><th>Nivel</th><th>Persona</th><th><?= $reportDomain === 'tests' ? 'Test / proceso' : 'Evaluación / proceso' ?></th><th>Señales</th><th>Estado</th><th class="text-end">Acción</th></tr></thead><tbody>
