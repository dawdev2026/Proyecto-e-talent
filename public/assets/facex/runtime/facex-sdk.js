...
class FaceXSDK {
  ...
  async load() {
    ...
    this._det = await FaceDetModule({
      locateFile: (path, scriptDirectory) => path.endsWith('.wasm') && window.FaceXDetectorWasm
        ? window.FaceXDetectorWasm
        : scriptDirectory + path
    });

    this._progress('Loading embedding engine...');
    this._fx = await FaceXModule({
      locateFile: (path, scriptDirectory) => path.endsWith('.wasm') && window.FaceXEmbeddingWasm
        ? window.FaceXEmbeddingWasm
        : scriptDirectory + path
    });
  ...
  async _loadCached(url, key) {
    ...
    try {
      ...
      if (cached && this._hasExpectedMagic(new Uint8Array(cached), key)) return new Uint8Array(cached);
      if (cached) {
        await new Promise(resolve => {
          const tx = db.transaction('weights', 'readwrite');
          tx.objectStore('weights').delete(key);
          tx.oncomplete = tx.onerror = tx.onabort = resolve;
        });
      }
    } catch(e) {
      // El almacenamiento local es opcional; la descarga validada sigue siendo suficiente.
      db = null;
    }

    const response = await fetch(url, { cache: 'no-store' });
    if (!response.ok) throw new Error(`No se pudo descargar el modelo facial (HTTP ${response.status}).`);
    const buffer = await response.arrayBuffer();
    const weights = new Uint8Array(buffer);
    if (!this._hasExpectedMagic(weights, key)) {
      throw new Error('El archivo del modelo facial recibido no corresponde al motor instalado.');
    }
    if (db) {
      try {
        const tx = db.transaction('weights', 'readwrite');
        ...
        // Un fallo de caché no debe impedir usar el modelo ya descargado.
      }
    }
    return weights;
  }

  _hasExpectedMagic(bytes, key) {
    const magic = key === 'facex_det_weights' ? 'DET8'
      : (key === 'facex_emb_weights' ? 'EFXS' : '');
